package com.yizhaoqi.searchSmart.controller;

import com.yizhaoqi.searchSmart.exception.CustomException;
import com.yizhaoqi.searchSmart.model.User;
import com.yizhaoqi.searchSmart.repository.UserRepository;
import com.yizhaoqi.searchSmart.service.UserService;
import com.yizhaoqi.searchSmart.utils.JwtUtils;
import com.yizhaoqi.searchSmart.utils.LogUtils;
...
public class UserController {
    ...
    */
    ...
    @PostMapping("/register")
        // ResponseEntity<?> 是 spring 框架的 HTTP 响应封装类
            // ResponseEntity 表示HTTP响应, <?>表示泛型
...
// 用户请求记录类
    // record 表示数据传输对象, 用于接收前端传递的数据
record UserRequest(String username, String password) {}

// 主组织标签请求记录类
record PrimaryOrgRequest(String primaryOrg) {}

// record 方法是用于创建数据载体类的一个新方法: 可以自动生成private final字段(常量)、全参数构造函数、Getter方法、toString()方法之类的
    // 使用直接: String password = request.password(); 即可