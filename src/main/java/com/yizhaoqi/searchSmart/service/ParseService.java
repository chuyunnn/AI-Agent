...
import com.yizhaoqi.searchSmart.model.DocumentVector;
import com.yizhaoqi.searchSmart.repository.DocumentVectorRepository;
...
public class ParseService {
    ...
    @Value("${file.parsing.parent-chunk-size:1048576}") //1MB
    private int parentChunkSize;
    
    @Value("${file.parsing.buffer-size:8192}") //8KB
     ...
     * 兼容旧版本的解析方法
     */
    // 方法重载，就是名称相同，参数列表不同 【编译时多态】
        // 其他：方法覆盖，子类重新实现父类方法，参数列表和名称都相同 【运行时多态】
    ...
    * */
    private void checkMemoryThreshold() {
        ...
        if (memoryUsage > maxMemoryThreshold) {
            ...
            System.gc(); // Full GC 回收新老年代、元空间（是一个建议操作，可能会不执行）
