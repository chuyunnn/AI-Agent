package com.yizhaoqi.searchSmart.service;

import com.yizhaoqi.searchSmart.exception.CustomException;
import com.yizhaoqi.searchSmart.model.OrganizationTag;
import com.yizhaoqi.searchSmart.model.User;
import com.yizhaoqi.searchSmart.repository.OrganizationTagRepository;
import com.yizhaoqi.searchSmart.repository.UserRepository;
import com.yizhaoqi.searchSmart.utils.PasswordUtil;
...
public class UserService {
    ...
    public void registerUser(String username, String password) {
        ...
        // 保存用户以生成ID
            // userRepository是数据访问层, 负责和数据库交互. 如果没有这个类就要自己写jdbc保存
        userRepository.save(user); // 通过userRepository保存到数据库中, 不用写实现类, 自动保存
    ...
    private void ensureDefaultOrgTagExists() {
        if (!organizationTagRepository.existsByTagId(DEFAULT_ORG_TAG)) { // 找是否存在默认租住标签
            ...
            defaultTag.setTagId(DEFAULT_ORG_TAG); // 标签唯一标识
            defaultTag.setName(DEFAULT_ORG_NAME); // 标签名称
            defaultTag.setDescription(DEFAULT_ORG_DESCRIPTION); // 标签描述
            defaultTag.setCreatedBy(creator); // 创建者
    ...
    public Map<String, Object> getUserList(String keyword, String orgTag, Integer status, int page, int size) {
        ...
        // 创建分页请求
            // pageIndex: 第几页, size: 每页几条, Sort.by(...)...: 按时间顺序排序（最新的在前面）
