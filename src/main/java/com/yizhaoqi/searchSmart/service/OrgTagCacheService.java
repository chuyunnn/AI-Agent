...
public class OrgTagCacheService {
    ...
    private static final String USER_ORG_TAGS_KEY_PREFIX = "user:org_tags:"; // 用户直接关联的标签
    private static final String USER_PRIMARY_ORG_KEY_PREFIX = "user:primary_org:"; // 用户所属的主组织
    private static final String USER_EFFECTIVE_TAGS_KEY_PREFIX = "user:effective_org_tags:"; // 经过计算的最终生效标签（含父级）
    ...
    public List<String> getUserEffectiveOrgTags(String username) {
        try {
            // 从缓存获取 USER_EFFECTIVE_TAGS_KEY_PREFIX 父级标签
            // 1. 尝试从 Redis 直接读取已计算好的有效标签（性能最高） match一个就行，不如match父标签
